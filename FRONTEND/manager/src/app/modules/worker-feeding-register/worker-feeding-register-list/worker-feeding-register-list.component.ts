...
export class WorkerFeedingRegisterListComponent implements OnInit {
  ...
  getFeedings(page :number) 
  {
    ...
    this.page = page
    this.feedingsService.getFeedings(this.page, this.size, this.groupcode, this.idpatient, this.day).subscribe(
      res => {
        this.isProcessing = false;
  ...
  getExportData(format: string) 
  {
    ...
    this.feedingsService.getFeedings(0, 100000000, this.groupcode, this.idpatient, this.day).subscribe(
      res => {
        const header = {};
  ...
  action(event) 
  {
    if (event && event[0] === 'edit') 
    {
      ...
      console.log("selected=" + JSON.stringify(this.feedingsObjects));
