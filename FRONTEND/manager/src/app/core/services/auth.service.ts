...
export class AuthService {
  ...
  requestResetPassword(email: string, username: string) {
    const form = new FormData();
    if (email) { form.append('email', email); }
    if (username) { form.append('username', username); }

    return this.http.post<any>(ENV.url.auth + `requestResetPassword`, form, {responseType: 'text' as 'json'});
  }

  getUserImg() {
    return this.localStorageService.getObject('userImg');
  }
